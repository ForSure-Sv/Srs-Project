...
int main()
{
	...
	if (choice == "2")
	{
		cout << "-----------Welcome to the Technician window-----------\n";
		cout << "Enter your Id" << endl;
		cin >> E_U;
		cout << "Enter Your password" << endl;
		Pass = CheckPassword();
		if (Technician_Login(E_U,Pass) == true)
		{
			cout << "Welcome " << GetName(E_U) << " to " << Get_Class(E_U)<< " class "<< endl;
		//to do technician menu
		}
	}
